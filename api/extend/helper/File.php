...
trait File
{
    ...
    public static function hasFile($filename, $storage = null)
    {
        switch (empty($storage) ? sysconf('storage_type') : $storage) {
            case 'local':
                return file_exists(env('root_path') . 'upload/' . $filename);
            case 'qiniu':
    ...
    public static function readFile($filename, $storage = null)
    {
        switch (empty($storage) ? sysconf('storage_type') : $storage) {
            case 'local':
                $file = env('root_path') . 'upload/' . $filename;
    ...
    {
        try {
            $realfile = env('root_path') . 'upload/' . $filename;
            ...
            if (file_put_contents($realfile, $content)) {
                ...
                return ['file' => $realfile, 'hash' => md5_file($realfile), 'key' => "upload/{$filename}", 'url' => $url];
            }
        } catch (Exception $err) {
    ...
    public static function download($url, $isForce = false)
    {
        try {
            ...
            if (false === $isForce && ($siteUrl = self::getFileUrl($filename, 'local'))) {
                $realfile = env('root_path') . 'upload/' . $filename;
                return ['file' => $realfile, 'hash' => md5_file($realfile), 'key' => "upload/{$filename}", 'url' => $siteUrl];
            }

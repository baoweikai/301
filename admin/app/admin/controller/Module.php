...
class Module extends \app\admin\Controller
{
    ...
    public function initialize()
    {
        ...
        $this->result['pattern'] = json_encode($field_pattern,true);
    }
    ...
    if(Request::isAjax()){
        if(input('isajax')) {
            ...
            if (input('name')) {
                ...
                $this->result['fieldtype'] = $fieldtype;
        ...
        $this->result['moduleid'] = $moduleid;
        $this->result['title'] = "添加字段";
        $this->result['info'] = 'null';
...
public function fieldEdit(){
    if(Request::isAjax()){
        ...
        $this->result['info'] = json_encode($fieldInfo,true);
        $this->result['title'] = "编辑字段");
        $this->result['moduleid'] = input('param.moduleid');
        return $this->fetch('fieldForm');
    }
}

<template>
  <div>
    ...
    <List
      ref="list"
      v-bind="{ columns: pick(columns, listItems), controller: controller, data: list, queryParams: queryParams, batch: ['add', 'audit'] }"
  ...
  data () {
    return {
      ...
      columns: {
        ...
        cate_id: {
          title: '分类', type: 'radio', rules: [{ required: true }], options: {}, width: 300
        },
        ...
        desc: { title: '描述', type: 'textarea', maxLength: 255, rules: [{ required: false }] },
        content: { title: '内容', type: 'editor', label: false, rules: [{ required: true }] },
        exame_status: {
          title: '审核状态', type: 'state', options: ['未审核', '审批中', '通过', '未通过'], rules: [{ required: true }]
        },
        status: {
      ...
      // 列表项
      listItems: ['id', 'title', 'cate_id', 'tagIds', 'exame_status', 'status', 'cover', 'creater_name', 'create_at', 'update_at', 'action'],
      // 表单

  ...
  data () {
    return {
      ...
      columns: {
        ...
        cate_id: { title: '分类', type: 'radio', options: {}, rules: [{ required: true }] },
        ppvod_key: { title: '转码服务器验证key', type: 'string', rules: [{ required: true }] },
        is_close: { title: '状态', type: 'radio', options: ['启用', '关闭'] },
        create_at: { title: '添加日期', type: 'date', format: 'MM-DD' },
        action: { title: '操作', type: 'action', actions: ['edit', 'del'] }
      },
      // 列表项
      listItems: ['id', 'title', 'host', 'cate_id', 'ppvod_key', 'is_close', 'action'],
      // 表单
      formItems: ['title', 'cate_id', 'title', 'host', 'ppvod_key', 'is_close']
    }
  },
  created () {
  },
